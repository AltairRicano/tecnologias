...
class SistemaDePoderes
	def consumir_hongo(jugador)
		...
		puts "[PODER] ¡Mario comió el hongo y ahora es GRANDE!"
	end 
end
...
puts "----INICIA EL NIVEL 1.1----"
puts "Mario tiene #{mario.vidas} vidas y estado es: #{mario.estado}."

puts "--- Escenario 1: El hongo y la emboscada--"
poderes.consumir_hongo(mario)
audio.reproducir(:poder)

print "¿Cuántos goombas emboscan a mario?: "
...
if mario.vidas > 0 
	...
	print "¿Cuántas veces golpea Borser a Mario?:  "
	cantidad_bowser = gets.chomp.to_i 
	cantidad_bowser = 0 if cantidad_bowser < 0 

	if cantidad_bowser > 0 
		dano_bowser = cantidad_bowser * bowser.dano_ataque
		puts "Bowser nos atacó #{cantidad_bowser} y nos hizo un daño de #{dano_bowser}"
